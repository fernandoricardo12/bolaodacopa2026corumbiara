...
BEGIN
  ...
  -- Compute accumulated_in only if this match is featured (acumulação só vale entre jogos em destaque)
  IF m_featured THEN
    -- Encontra último jogo destaque anterior que TEVE ganhador (reset da acumulação)
    ...
    -- Soma 80% do bolo dos jogos destaque anteriores SEM ganhador, posteriores ao reset
  ...
  IF exact_amount_total > 0 THEN
    -- Prêmio principal: (80% do bolo atual + acumulado), proporcional ao valor apostado por quem cravou
    UPDATE public.individual_bets
      SET payout = ((total_pool * 0.80) + accumulated_in) * (amount / exact_amount_total)
      WHERE match_id = _match_id AND paid = true
        AND home_score = result_home AND away_score = result_away;

    -- Bônus extra: dividido igualmente entre quem cravou
    IF bonus > 0 THEN
  ...
  -- Sem ganhador: 80% do bolo acumula automaticamente para o próximo jogo destaque
  -- (computado dinamicamente quando o próximo destaque for processado)
END
$function$;
